...
- Automatic application of schemas with prefixes "00" and "01". Since `206a1f0` that is true
...
- Schema files must follow `create_*.sql` naming convention, even when they ALTER
  (`create_cookies_expires_at.sql`): `Database\SchemaPath::resolve()` admits only that prefix
